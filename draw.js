...
const drawLineOfSight = (g, sw, sh, m) => {
  ...
  if (m && !input) {
    ...
    const i = closest(target, [
      ...intersections(target, m, 0, 0, sw, sh),
      ...boxes.flatMap(b => intersections(target, m, b.x - b.w, b.y - b.h, b.w * 2, b.h * 2)),
      ...circles.flatMap(c => circleSegmentIntersection(target.x, target.y, m.x, m.y, c.x, c.y, c.r))
    ]);
    if (i) {
      g.fillStyle = magenta;
      g.dot(i, 8).fill();
